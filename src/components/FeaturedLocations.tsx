...
export default async function FeaturedLocations() {
  ...
  // Fetch top 3 approved hoardings
  const hoardings = await Hoarding.find({ status: 'approved' })
    .sort({ createdAt: -1 })
    .limit(3)
    .lean();

  if (!hoardings || hoardings.length === 0) {
    return null; // Don't show the section if no hoardings are found
  }

  return (
    <section className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-10 pb-16">
      <div className="flex justify-between items-end mb-8">
        <h2 className="text-[28px] font-bold text-[#0f172a] tracking-tight font-display">Featured Locations</h2>
        <Link 
          href="/explore" 
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {hoardings.map((loc: any) => (
          <div key={loc._id.toString()} className="flex flex-col bg-white rounded-xl border border-gray-200 overflow-hidden hover:shadow-lg transition-shadow duration-300">
            {/* Image */}
            <div className="relative h-[220px] w-full bg-gray-100">
              <img 
                src={loc.images?.[0] || "https://images.unsplash.com/photo-1559828330-9b3de57c1692?auto=format&fit=crop&q=80&w=800"}
                alt={loc.name}
            ...
            <div className="p-5 flex flex-col flex-grow">
              <div className="flex items-center gap-1.5 text-slate-800 font-bold mb-1">
                ...
                <span className="text-[15px]">{loc.location.city}, {loc.location.area}</span>
              </div>
              ...
              <div className="mt-auto">
                <Link 
                  href={`/hoardings/${loc._id.toString()}`}
